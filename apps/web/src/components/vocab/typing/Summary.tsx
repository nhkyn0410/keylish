...
function StatBlock({ value, label, sub, bg }: { value: string; label: string; sub?: string; bg: string }) {
  return (
    <div className="k-card" style={{ flex: 1, padding: "22px 20px", background: bg, textAlign: "center" }}>
      <div className="k-stat-num" style={{ fontSize: 62 }}>{value}</div>
      <div style={{ fontWeight: 900, fontSize: 14, textTransform: "uppercase", letterSpacing: ".1em", marginTop: 6 }}>{label}</div>
      {sub && <div style={{ fontSize: 12, fontWeight: 700, opacity: 0.6, marginTop: 3 }}>{sub}</div>}
    </div>
  );
}

export function Summary({
  result, contextLabel, onReviewWrong, onRetry, onChangeMethod,
}: {
    ...
    <div className="k-screen">
      ...
      <div style={{ flex: 1, paddingTop: 30, paddingBottom: 30, position: "relative", minHeight: 0, overflow: "auto" }}>
        ...
        <div style={{ display: "flex", gap: 18 }}>
          <StatBlock value={String(result.wpm)} label="WPM" sub="tốc độ gõ" bg="var(--neo-yellow)" />
          <StatBlock value={result.accuracyPct + "%"} label="Chính xác" sub={`${result.correctChars} / ${result.totalChars} ký tự`} bg="var(--neo-violet)" />
          <StatBlock value={String(result.correct)} label="Đúng" sub="từ đã thuộc" bg="var(--neo-green)" />
          <StatBlock value={String(result.wrong)} label="Sai" sub="cần ôn lại" bg="var(--neo-red)" />
        </div>

        <div className="k-card" style={{ padding: 24, flex: 1, minHeight: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
            <div className="k-badge k-badge--red"><Icon name="refresh" size={15} stroke={3} /> Từ sai · lặp lại (M9)</div>
          ...
          {hasWrong ? (
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              {result.wrongWords.map((w) => (
                <div key={w.en} className="k-b2 k-sh-sm" style={{ padding: "16px 18px", display: "flex", alignItems: "center", gap: 14, background: "#fff", minWidth: 220 }}>
            ...
            <div className="k-b2" style={{ padding: "18px 20px", background: "var(--neo-green-soft)", boxShadow: "none", fontWeight: 800, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
          <button type="button" className="k-btn k-btn--primary k-btn--lg" onClick={onReviewWrong} disabled={!hasWrong}>
            <Icon name="refresh" size={20} /> Ôn từ sai ({result.wrongWords.length})
          </button>
          <button type="button" className="k-btn k-btn--info" onClick={onRetry}><Icon name="replay" size={18} /> Làm lại</button>
