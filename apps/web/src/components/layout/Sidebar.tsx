...
function NavIcon({ name, size = 20, stroke = 3 }: { name: string; size?: number; stroke?: number }) {
  ...
  const paths: Record<string, ReactNode> = {
    home: <g {...p}><path d="M4 11l8-7 8 7" /><path d="M6 10v9h12v-9" /></g>,
    compass: <g {...p}><circle cx="12" cy="12" r="8.5" /><path d="M15.5 8.5l-2.2 5.3-5.3 2.2 2.2-5.3z" /></g>,
    sentence: <g {...p}><path d="M4 6h16M4 11h16M4 16h9" /></g>,
    list: <g {...p}><path d="M9 6h11M9 12h11M9 18h11" /><path d="M4.5 6h.01M4.5 12h.01M4.5 18h.01" strokeWidth={stroke + 1} /></g>,
  };
...
function SideLink({ href, label, icon, custom, active }: { href: string; label: string; icon: string; custom?: boolean; active: boolean }) {
  return (
    <Link href={href} className={"k-side-item " + (active ? "k-side-item--active" : "k-side-item--idle")} aria-current={active ? "page" : undefined}>
  ...
  return (
    <aside className="k-sidebar" aria-label="Thanh điều hướng">
      ...
      <nav className="k-side-nav" aria-label="Điều hướng chính">
        ...
        <SideLink href={exploreHref} label="Khám phá" icon="compass" custom active={isActive(exploreHref)} />
        ...
        <SideLink href={sentenceHref} label="Luyện câu" icon="sentence" custom active={isActive(sentenceHref)} />
        <div className="k-side-divider" />
        <SideLink href={vocabularyHref} label="Kho từ vựng" icon="book" active={isActive(vocabularyHref)} />
        <SideLink href={lessonsHref} label="Quản lý bài" icon="list" custom active={isActive(lessonsHref)} />
      </nav>
