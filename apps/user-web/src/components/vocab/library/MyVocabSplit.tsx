...
import type { CefrLevel, TopicDTO, UserVocabEntryDto } from "@keylish/shared";
...
export function MyVocabSplit() {
      ...
      fetchUserVocab({
        ...
        .then((res) => {
          ...
          setSelId((current) => {
            ...
            const nextIds = new Set(res.items.map((entry) => entry.id));
            return nextIds.has(current) ? current : null;
          });
        })
        .catch((err) => {
          if (!active) return;
  ...
  }, [levelList, reloadKey, search, topicList]);
  ...
  function toggle<T extends string>(set: Set<T>, setter: (next: Set<T>) => void, value: T) {
  ...
  if (authError) {
    return (
      <div
        className="k-screen k-b"
        style={{ height: "100%", minHeight: 360, alignItems: "center", justifyContent: "center" }}
      >
        <div
          className="k-card"
          style={{ padding: "28px 30px", textAlign: "center", maxWidth: 420 }}
        >
      ...
      <div style={{ flex: 1, minHeight: 0, display: "flex" }}>
        ...
        >
          ...
          <div style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
            <button
              ...
              style={{ width: "100%" }}
            >
              Luyện kho này <Icon name="arrow" size={16} />
            </button>
            <div style={{ fontSize: 12, fontWeight: 700, opacity: 0.55, lineHeight: 1.4 }}>
              {total} từ khớp bộ lọc
              <br />
              {entries.length} đang hiển thị
            </div>
          </div>
        </div>
        ...
        >
          <div className="k-h-eyebrow" style={{ color: "#7a6a00" }}>
            Kho của tôi {loading ? "· đang nạp…" : `· ${total}`}
          </div>
          ...
          {!loading && filtered.length === 0 ? (
            <div
              className="k-card"
              style={{ padding: 18, fontWeight: 700, opacity: 0.75, lineHeight: 1.5 }}
            >
              Không có từ nào khớp bộ lọc. Thêm từ ở <strong>Kho hệ thống</strong>, bấm{" "}
              <strong>Tạo từ mới</strong>, hoặc bỏ bớt cấp độ / chủ đề.
            </div>
          ) : null}
              ...
              >
                ...
                <span
                  ...
                  style={{
                    ...
                    flex: "0 0 auto",
                    background:
                      e.source === "custom" ? "var(--neo-violet-soft)" : "var(--neo-white)",
                    boxShadow: "none",
                  }}
                >
            ...
            <>
              ...
              <div style={{ display: "flex", gap: 10, position: "relative", zIndex: 1 }}>
                ...
                <button
                  ...
                  onClick={() =>
                    router.push(
                      practiceHref(makeLibrarySpec({ source: "personal", search: enOf(sel), size: 1 }))
                    )
                  }
                  style={{ flex: 1 }}
                >
...
type Suggestion = { lemma: string; candidates: Candidate[] };
  ...
  async function submit(allowVariant: boolean) {
    ...
    try {
      ...
      if (res.status === "suggest") {
        setSuggest({ lemma: res.lemma, candidates: res.candidates });
        return;
      }
      onDone(); // linked | created
    } catch (err) {
      if (err instanceof ApiError && err.status === 401)
        setError("Đăng nhập để lưu vào kho của bạn.");
      else if (err instanceof ApiError && err.status === 409)
        setError("Từ này đã có trong kho của bạn.");
      else setError(getErrorMessage(err));
    } finally {
      setSubmitting(false);
    }
        ...
        {suggest ? (
          <div>
            <p style={{ fontWeight: 700, fontSize: 15, lineHeight: 1.5, marginBottom: 12 }}>
              “{en.trim()}” có thể là biến thể của “{suggest.lemma}” trong kho hệ thống. Dùng từ
              gốc?
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
              {suggest.candidates.map((c) => (
                ...
                >
                  ...
                  <span style={{ fontWeight: 700, fontSize: 13, opacity: 0.7, flex: 1 }}>
                    {c.vi}
                  </span>
            ...
            {error ? (
              <p
                style={{ fontSize: 12, fontWeight: 800, color: "var(--neo-red)", marginBottom: 10 }}
              >
                {error}
              </p>
            ) : null}
