...
|---|---|---|---|
| Session token | Đánh cắp cookie (XSS) | `httpOnly` cookie — JS không đọc được | ✅ |
| Session token | Man-in-the-Middle | `Secure` flag (production), `SameSite=None` cross-site | ✅ |
| CSRF | Cross-site request forgery | Double-submit cookie + Origin check | ✅ |
| Password | Brute force | Rate limiting (in-memory) | ✅ (partial — mất khi restart) |
| Password | Database leak | Argon2id hash — configurable cost | ✅ |
| Token | Database leak | HMAC-SHA256 + pepper — không lưu raw | ✅ |
| User enumeration (login) | Timing attack | Dummy Argon2 verify cho account ảo | ✅ |
| User enumeration (register) | Error message | `ConflictException` — không tránh được hoàn toàn | ⚠️ (chấp nhận) |
| User enumeration (forgot) | Response difference | Luôn return `{ ok: true }` | ✅ |
| User enumeration (admin login) | Username format | Từ chối `@` trong username ngay cả khi username tồn tại | ✅ |
| IP tracking | Log IP thô | HMAC-SHA256(ip, pepper) — không reversible | ✅ |
| Admin surface | Public exposure | `AdminGateGuard` — 404 khi `ADMIN_API_ENABLED=false` (mặc định prod) | ✅ |
| Reset token | Token reuse | One-time (`usedAt`) + TTL 2 giờ | ✅ |
| Session reuse | Session theft | `revokedAt` check + `expiresAt` check | ✅ |
| Injection (SQL) | Query injection | Prisma ORM — parameterized queries | ✅ |
| Injection (NoSQL) | Zod schema | Type coercion + strict validation | ✅ |
...
|---|---|---|---|
| 1 | Argon2id password hash | ✅ | `auth.service.ts:235` |
| 2 | Configurable Argon2 cost | ✅ | env `AUTH_ARGON2_*` |
| 3 | Session token HMAC + pepper | ✅ | `auth.service.ts:84` |
| 4 | Cookie httpOnly (session) | ✅ | `auth.service.ts:155` |
| 5 | Cookie SameSite | ✅ | Dev=lax, Prod=none |
| 6 | Cookie Secure | ✅ | Prod + SameSite=None |
| 7 | CSRF double-submit | ✅ | `auth.guard.ts:47` |
| 8 | Origin/Referer check | ✅ | `auth.service.ts:886` |
| 9 | Rate limiting | ✅ | `auth.service.ts:934` (in-memory) |
| 10 | Timing attack protection | ✅ | `auth.service.ts:269` |
| 11 | No PII in logs (by convention) | ✅ | review required |
| 12 | IP hashed (not raw) | ✅ | `auth.service.ts:929` |
| 13 | Admin API gated (404 prod) | ✅ | `admin-gate.guard.ts` |
| 14 | Reset token one-time | ✅ | `auth.service.ts:667` |
| 15 | Reset token 2h TTL | ✅ | `auth.service.ts:625` |
| 16 | PEPPER bắt buộc prod | ✅ | `main.ts:11` |
| 17 | Password min 12 chars | ✅ | `auth.dto.ts:5` |
| 18 | Password max 128 chars | ✅ | `auth.dto.ts:5` |
| 19 | Login anti-enumeration | ✅ | `auth.service.ts:269` (timing) |
| 20 | Admin login anti-email | ✅ | `auth.service.ts:425` |
| 21 | Forgot-password no-leak | ✅ | luôn `{ ok: true }` |
| 22 | Session background purge | ✅ | `auth.service.ts:215` |
| 23 | Token rotation on password change | ✅ | `auth.service.ts:542` |
| 24 | Module-level guard (AdminGuard) | ✅ | controller `@UseGuards` |
| 25 | Global admin gate (APP_GUARD) | ✅ | `app.module.ts` |
| 26 | CORS whitelist | ✅ | `main.ts` |
| 27 | Zod validation all inputs | ✅ | schemas in services |
| 28 | trust proxy setting | ✅ | `main.ts` for real IP |
| 29 | Prisma parameterized queries | ✅ | ORM built-in |
| 30 | Soft-delete user (not hard) | ✅ | `User.deletedAt` |
| 31 | Cascade delete on user delete | ✅ | migration SQL |
| 32 | Non-sequential PK (cuid) | ✅ | Prisma schema |
| 33 | CSRF skip safe methods | ✅ | `auth.service.ts:335` |
| 34 | Cookie `__Host-` prefix (prod) | ✅ | `auth.service.ts:50` |
| 35 | Session touch interval (10 min) | ✅ | `auth.service.ts:847` |
