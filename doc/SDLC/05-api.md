...
| 0.2.3     | 2026-06-20 | AI Agent       | Sync as-built: public vocab có `search`/`offset`; thêm UserVocabModule và 5 endpoint `/api/user/vocab*` đã code.                                                           |
| 0.2.4     | 2026-06-20 | AI Agent       | Bổ sung query `levels/topics` cho `/api/user/vocab`; custom vocab có `topic` tùy chọn.                                                                                     |
...
└── UserVocabModule       ← DatabaseModule, AuthModule
```
...
## 3. Route map — tổng thể 38 endpoints
...
### 3.8. User Vocab (5 endpoints — session/csrf)

| Method | Path                   | Guards               | Mô tả                         | §   |
| ------ | ---------------------- | -------------------- | ----------------------------- | --- |
| GET    | `/api/user/vocab`      | UserGuard            | Liệt kê kho cá nhân           | 13  |
| POST   | `/api/user/vocab/pick` | UserGuard, CsrfGuard | Thêm tham chiếu từ hệ thống   | 13  |
| POST   | `/api/user/vocab`      | UserGuard, CsrfGuard | Tạo từ cá nhân + dedup-on-add | 13  |
| PATCH  | `/api/user/vocab/:id`  | UserGuard, CsrfGuard | Sửa override/note             | 13  |
| DELETE | `/api/user/vocab/:id`  | UserGuard, CsrfGuard | Xóa entry                     | 13  |
...
| `search`    | `z.string().trim().max(120)`                      | undefined | Tìm trong `en` hoặc `vi`                 |
| `limit`     | `z.coerce.number().int().min(1).max(100)`         | 20        | Max items                                |
| `offset`    | `z.coerce.number().int().min(0).max(100000)`      | 0         | Phân trang public vocabulary             |
...
```typescript
const where = publicVocabWhere(parsed); // levels/topics/search
// Non-random: orderBy frequency desc, en asc; limit + offset applied to Prisma query
...
**Validation**: `VocabQuerySchema` — dùng `levels` + `topics` + `search`, bỏ qua `limit` + `offset` + `random`.

**Query logic** (`vocab.service.ts:133`):

```typescript
prisma.word.count({ where: publicVocabWhere(parsed) });
```
...
## 13. (V2.1 PARTIAL — đã code partial) API kho từ vựng cá nhân

Mọi endpoint qua `UserGuard`; thao tác ghi thêm `CsrfGuard`; service luôn lọc/ghi theo `userId` lấy từ phiên, không nhận `userId` từ client (NFR-SEC-05). Versioning theo D-07 (giữ as-built — `/api/user/*`, không `/v1`). UI sửa/override chưa nối đầy đủ; FR-PVOC-08 nằm ngoài module này.
...
| POST   | `/api/user/vocab`      | UserGuard, CsrfGuard | Tạo từ (`{ en, vi, example?, level?, topic?, note?, allowVariant? }`) → **dedup-on-add** | FR-PVOC-03/04/05 |
...
### 13.1. Query list (`GET /api/user/vocab`)

| Query      | Validation        | Default   | Ghi chú                                              |
| ---------- | ----------------- | --------- | ---------------------------------------------------- |
| `search`   | `string.trim()`   | undefined | Tìm trong `customEn/customVi` hoặc `word.en/word.vi` |
| `levels`   | CSV/list `A1..C2` | undefined | Match `customLevel` hoặc `word.level`                |
| `topics`   | CSV/list slug     | undefined | Match `customTopic.slug` hoặc `word.topic.slug`      |
| `page`     | int ≥1            | 1         | Phân trang kho cá nhân                               |
| `pageSize` | int 1..100        | 50        | UI dùng 100 + nút tải thêm                           |

### 13.2. Dedup-on-add (`POST /api/user/vocab`)

1. `normalizedEn = normalizeEn(en)` (`trim`, lowercase, chỉ giữ `[a-z'-]`).
...
4. Nếu client gửi `allowVariant=true`, bỏ qua gợi ý biến thể và tạo custom riêng.
5. Không khớp → tạo custom (`topic` nếu có là topic slug) → `{ status: "created", entry }`.
6. Trùng trong kho → `409` (BR-09).

### 13.3. Response shape (Zod ở `@keylish/shared`)

`UserVocabEntryDto = { id, source, word?: WordDTO|null, custom?: { en, vi, example?, level?, topic? }|null, note?, createdAt }`.

`AddVocabResult = { status:"linked", entry } | { status:"created", entry } | { status:"suggest", lemma, candidates }`.

Mã lỗi theo §8 (400/401/403/409).
